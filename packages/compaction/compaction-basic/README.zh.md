...
摘要模型会接收逐字回放的会话：与上次已路由请求为已遮蔽区域发送的相同系统提示词、工具 schema 与消息，其中图片块会被替换为稳定的文本占位符，后面跟随一条最终 user 消息，即下方压缩指令。图片字节从不上摘要请求，因此被污染的历史无法污染自己的救援。会话模型绝不会看到该私有请求或其推理；只有返回文本会被存储。
