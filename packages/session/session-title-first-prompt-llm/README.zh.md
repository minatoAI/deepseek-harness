...
失败的生成——主请求之前缺少路由、`maxInputBytes` 小于空封装、超时、取消或无效模型输出——会发出警告并保留当前标题；只有显式 `refresh()` 会重试。超限的首消息会截断为前部前缀而不是失败。自动工作不会为主 agent（智能体）请求增加 token 或延迟。
...
标题模型会收到共享标题指令，以及一个只包含第一条符合条件用户消息的 JSON 数组；超限时会截断为前部前缀。后续提示词与继承的 fork 历史不会触发再次自动调用。
