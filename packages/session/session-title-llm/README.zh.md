...
`provider` 与 `model` 覆盖项都是可选的，但必须同时作为非空字符串提供。如果没有这一对取值，辅助函数使用当前会话已记录 `request/header` 中捕获的确切提供方／模型路由，因此在任何路由出现前显式刷新时必须提供覆盖项。辅助函数在记录或分发前，把最终 JSON 封装用户提示词装入 `maxInputBytes`（单条消息保留前部前缀；多条消息保留首条与末条并丢弃中间，必要时截断前缀），并在消费流期间与完成后重新检查超时与调用方取消，因此即使拦截器或适配器忽略 abort，也不能接受迟到的成功结果。只有小于空封装的预算才会拒绝。格式错误或空输出、工具调用与非 stop 结束原因都会拒绝；会话标题服务决定该拒绝属于自动警告还是显式调用方失败。
...
| `maxInputBytes` | 必填 | 最终 JSON 封装用户提示词的 UTF-8 字节截断预算 |
...
标题模型会收到固定系统指令，要求以输入语言返回一个简洁且无装饰的标题；该指令包含所配置的词数与 CJK 字符数目标。它唯一的用户消息包含一个 JSON 数组，其中是装入预算后的用户消息及其 seq：能装下时为完整选择，否则为单条前部前缀或丢弃中间后的首／末条。

#### Token 影响

辅助请求根据所选输入大小与 `maxOutputTokens` 消耗 token。它与主 agent 请求相互独立，不会向 agent 历史增加标题文本或封装内容。DeepSeek 标题调用会关闭思考，pi-ai 适配器将标题调用解析为最低支持的 thinking 等级（模型提供 off 时即为 off）；主对话保留自身配置的思考模式。
...
- **前缀／首尾截断**——超限输入保留单条前部前缀，或保留首／末条并丢弃中间；截断后的前缀不带省略号标记。
