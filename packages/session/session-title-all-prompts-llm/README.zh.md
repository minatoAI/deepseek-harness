...
每条新的符合条件用户提示词之后都会启动新修订，包括子会话中的提示词；生成会折叠截至当前修订的所有符合条件消息，预置历史也包含在内。较新的修订会中止并取代旧工作，因此陈旧的完成结果永远无法提交。超限输入保留首条与末条并丢弃中间（必要时截断前缀），使标题同时记住背景与最新进展；自动失败会发出警告并保留先前标题；`ctx.sessionTitle.refresh()` 是显式重试。
...
如果最终封装的聚合提示词超过 `maxInputBytes`，请求会保留首／末条并截断前缀而不是失败；只有小于空封装的预算才会失败。自动使用时失败会发出警告并保留先前标题，只有显式 `refresh()` 会重试。自动工作不会为主 agent 请求增加 token 或延迟。
...
标题模型会收到共享标题指令，以及一个 JSON 数组，其中按日志顺序包含装入预算后的用户消息和确切 seq：能装下时为全部消息，否则为丢弃中间后的首／末条。预置历史也包含在内。
...
- **首尾保留**——超限输入保留首／末条并截断前缀，不带省略号标记；没有基于摘要继续生成摘要的机制。
