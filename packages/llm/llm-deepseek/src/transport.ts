...
import { isContextWindowExceededError, isQuotaExceededError, isRegionUnsupportedError, LlmError, ProviderRequestId, REGION_UNSUPPORTED_CODE } from '@deepseek-ai/dsh-llm'
...
export function providerError(raw: unknown, status: number | undefined, headers?: Headers): LlmError {
  ...
  let code: string
  if (status === 401 || ['authentication_error', 'permission_error'].includes(type)) code = 'AUTH'
  else if (status === 403) code = isRegionUnsupportedError(detail) ? REGION_UNSUPPORTED_CODE : 'AUTH'
