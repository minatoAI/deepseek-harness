...
import { attributionHeaders, contentHasImage, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, isRegionUnsupportedError, LlmAdapter, LlmError, offloadedImageText, offloadRequestImagesWithPolicy, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId, REGION_UNSUPPORTED_CODE } from '@deepseek-ai/dsh-llm'
import type {
  ContentBlock,
  GenerateOptions,
...
export function httpErrorCode(status: number, error?: WireError['error']): string {
  ...
  if (status === 403) {
    const regionDetail = [error?.code, error?.type, error?.message].filter(Boolean).join(' ')
    if (isRegionUnsupportedError(regionDetail)) return REGION_UNSUPPORTED_CODE
    return 'AUTH'
  }
