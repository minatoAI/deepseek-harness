...
`reasoningEfforts` 声明模型可选的思考级别：每个键是选择器提供的一个档位，其值是分派在协议中发送的拼写，因此 `high: high` 原样透传规范名称，而 `max: ultra` 则为使用自有词汇的网关改名。键取自 pi-ai 的档位集合（`off`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max`）；未声明的档位不会被提供。保留键 `default` 不是档位：它点名该模型在请求未指定档位时使用的规范思考级别，仅对本模型覆盖路由级 `reasoning` 字段，且必须是该模型实际提供的档位。只含 `default` 的字典会保留已安装 catalog 条目的档位集合（手工声明的模型没有这份集合，因此会以该规范拼写单独提供所点名的默认档位），而不是被读成空声明——这就是「保留档位、指定默认值」的写法。省略该字段会保留已安装 catalog 条目的能力（手工声明的模型没有这份能力，也不推理）；`false` 声明一个不具备推理能力的模型，profile 正是以此从其网关无法服务的 catalog 模型上剥除推理；空声明会被拒绝，而不是在这两种含义之间去猜。
