...
/**
 * Accept an object root that composes branches instead of declaring
 * properties (for example a zod discriminated union rendered as `oneOf`
 * object branches): the composition must be a non-empty array of schema
 * objects, and every branch meets the same root contract, checked
 * recursively. Anything else throws a tool-named error.
 * @param name - the tool name, included in every error.
 * @param parameters - a schema whose root is `type: 'object'`.
 */
function assertCompositeRootParameters(name: string, parameters: Record<string, unknown>): void {
  const composite = parameters.oneOf ?? parameters.anyOf
  if (!isPlainJsonArray(composite) || composite.length === 0) {
  ...
  for (const branch of composite) {
    if (!isJsonSchemaRecord(branch)) {
      throw new Error(`tool "${name}": invalid parameters schema: every oneOf/anyOf branch must be a schema object. ${REGISTER_PARAMETERS_FIX_HINT}`)
    }
    assertObjectParameters(name, branch)
  }
}
