...
describe('normalizeRegisteredParameters', () => {
  ...
  it('passes an object root composing oneOf branches through byte-for-byte', () => {
    // Shipped discriminated-union inputs (for example a zod discriminated
    // union rendered as oneOf) carry no root properties; every object-rooted
    // branch meets the same property contract instead.
    const union = {
      type: 'object',
      oneOf: [
        {
  ...
  it('passes an anyOf-only composite root through byte-for-byte', () => {
    const union = {
      type: 'object',
      anyOf: [{ type: 'object', properties: { path: { type: 'string' } }, required: ['path'] }],
    }
    expect(normalizeRegisteredParameters('stagehand_tabs', union)).toBe(union)
  })
  ...
  it('rejects composite roots with no usable branches with a tool-named error', () => {
    ...
    expect(() => normalizeRegisteredParameters('stagehand_tabs', { type: 'object', oneOf: [42] }))
      .toThrow(/stagehand_tabs/)
    expect(() => normalizeRegisteredParameters('stagehand_tabs', {
      type: 'object',
      oneOf: [{ type: 'object', properties: { action: { type: 'string' } }, required: ['missing'] }],
    })).toThrow(/missing/)
  })
